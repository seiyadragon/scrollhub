...
export async function getServerSideProps(context) {
  ...
  let { data: pageCount, error2 } = await supabase
    .from("Books")
    .select()
    .eq("id", -69);
  ...
  return (
    <section className={styles.pageSelector}>
      <Link href={"/books?page=1"} scroll={false}>
        <span
          className={`${styles.pageSelectorItem} ${styles.pageSelectorItemOutside}`}
        >
          1
        </span>
      </Link>
      ...
      <span
        className={`${styles.pageSelectorItem} ${styles.pageSelectorItemCenter}`}
      >
        {page}
      </span>
